...
class Character():
    ...
    def level_up(self) -> None:
        while self.exp >= self.exp_to_next_level:
            ...
            # Pemain mendapatkan 3 poin stat
            self.allocate_stat_points(3)

    def allocate_stat_points(self, points: int) -> int:
        """Pemain mendistribusikan poin stat sesuai keinginan."""
        print(f"\n📈 Anda memiliki {points} poin untuk dialokasikan!")

        while points > 0:
            print("\nPilih stat untuk ditingkatkan:")
            print("1. ❤️ Max HP (+15 per poin)")
            print("2. ⚔️ Attack (+1 per poin)")
            print("3. 🏃 Agility (+1 per poin)")
            print("4. Max Mana (+20 per poin)")
            choice = input(f"Sisa poin: {points} | Pilih stat (1-4): ")

            if choice == "1":
                self.max_hp += 10
                self.hp = self.max_hp  # Isi ulang HP setelah naik level
                print(f"❤️ Max HP meningkat menjadi {self.max_hp}!")
                points -= 1
            elif choice == "2":
                self.attack += 2
                print(f"⚔️ Attack meningkat menjadi {self.attack}!")
                points -= 1
            elif choice == "3":
                self.defense += 1
                print(f"🛡️ Defense meningkat menjadi {self.defense}!")
                points -= 1
            elif choice == "4":
                self.agility += 1
                print(f"🏃 Agility meningkat menjadi {self.agility}!")
                points -= 1
            else:
